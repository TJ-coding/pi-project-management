...
| 22 | TUI is mandatory: dashboard + Direction/Goals/State/Intelligence/Risks/Strategy/Plan/History/Runs views | `dashboard.ts` (11 views incl. Summary), `ProjectBrowser` (single-line title/tab chrome, terminal-sized scroll viewport, line-range footer), widget + footer status | unit: dashboard (all views, width, navigation, chrome height, heading fit, scroll reporting); tui: real terminal in a pty with exact pyte frames, on both an empty and a content-rich project (including j/G/g scrolling) |
| 23 | `/project …` commands + natural language | `commands.ts` (all listed subcommands, `/pm` alias, print-mode output), tools + injected digest | tui/`rpc-commands` script (20 commands), e2e, `pi -p "/project status"` |
